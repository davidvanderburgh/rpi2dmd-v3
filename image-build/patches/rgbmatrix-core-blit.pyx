...
cdef class Canvas:
    ...
    def SetPixelsPillow(self, int xstart, int ystart, int width, int height, image):
        ...
        cdef int row, col
        cdef int col0 = max(0, -xstart)
        cdef int row0 = max(0, -ystart)
        cdef int col1 = min(width, frame_width - xstart)
        cdef int row1 = min(height, frame_height - ystart)
        ...
        # Row-outer iteration: image32 rows are contiguous in memory, so
        # this walks the buffer sequentially. The original col-outer loop
        # strided across rows (one cache miss per pixel on ARM11) and held
        # the GIL for the whole walk.
        with nogil:
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def SetImageBytes(self, const uint8_t[::1] buf, int width, int height,
                      int offset_x = 0, int offset_y = 0):
        """Blit raw RGB24 bytes (row-major, len >= width*height*3) to the
        canvas. Same result as SetImage(Image.frombytes('RGB', (w, h), buf))
        but with no PIL object, no per-pixel Python and no pointer casts:
        accepts bytes/bytearray/memoryview from any thread and reads it
        sequentially under nogil."""
        cdef cppinc.FrameCanvas* my_canvas = <cppinc.FrameCanvas*>self.__getCanvas()
        cdef int frame_width = my_canvas.width()
        cdef int frame_height = my_canvas.height()
        cdef int col0 = max(0, -offset_x)
        cdef int row0 = max(0, -offset_y)
        cdef int col1 = min(width, frame_width - offset_x)
        cdef int row1 = min(height, frame_height - offset_y)
        cdef int row, col
        cdef Py_ssize_t base
        if col1 <= col0 or row1 <= row0:
            return
        if buf.shape[0] < (<Py_ssize_t>height) * width * 3:
            raise ValueError("buffer too small: %d < %d"
                             % (buf.shape[0], height * width * 3))
        with nogil:
            for row in range(row0, row1):
                base = (<Py_ssize_t>row * width + col0) * 3
                for col in range(col0, col1):
                    my_canvas.SetPixel(offset_x + col, offset_y + row,
                                       buf[base], buf[base + 1],
                                       buf[base + 2])
                    base += 3
